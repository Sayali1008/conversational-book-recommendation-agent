...
from server.session_state import UserSessionState
...
storage = Interactions(PATHS["database"])
...
def update_pipeline_state(status: str = None, current_stage: str = None, overall_progress: int = None, error: str = None, pipeline_id: str = None):
...
def get_book_details(book_id: int):
    ...
    try:
        if not service.ready:
            raise HTTPException(status_code=503, detail="Recommendation artifacts are not available. Run the pipeline to generate them.")
...
def recommend(user_id: Optional[str] = None, k: int = 10, seen_book_ids: Optional[str] = None):
    try:
        if not service.ready:
            raise HTTPException(status_code=503, detail="Recommendation artifacts are not available. Run the pipeline to generate them.")

        user_cf_idx = service.get_user_idx(user_id)

        seen = []
        if seen_book_ids:
            seen = [int(s) for s in seen_book_ids.split(",") if s.strip()]
        seen_catalog_rows = service.book_ids_to_catalog_indices(seen)
        recs, strategy = service.recommend(user_cf_idx=user_cf_idx, k=k, exclude_catalog_rows=seen_catalog_rows)
        return RecommendResponse(
            recommendations=[BookRecommendation(**r) for r in recs],
            strategy=strategy,
            used_seeds=seen_catalog_rows,
        )
    except Exception as e:
        error_msg = f"{str(e)}\n{traceback.format_exc()}"
...
def swipe(payload: SwipeRequest):
    """
    Event logger for user swipes (likes/dislikes).

    Flow:
    1. Normalize confidence: 1.0 for "like", 0.0 for "dislike"
    2. Log interaction to SQLite for audit trail + future batch retraining
    3. Update lightweight session state (recent_likes, recent_dislikes)
    4. Generate fresh batch of recommendations using recent likes as seeds
    5. Return new recommendations to replace old ones (prevents clutter)

    NOTE: Swipes do NOT trigger model retraining, CF factor updates, or embedding rebuilds.
    They only update lightweight session state and guide content-based recommendations.
    """
    try:
        if not service.ready:
            raise HTTPException(status_code=503, detail="Recommendation artifacts are not available. Run the pipeline to generate them.")
        ...
        # Step 1: Log interaction to persistent storage
        storage.log_swipe(payload.user_id, payload.book_id, payload.action, confidence)

        # Step 2: Update lightweight session state
        session_state.record_swipe(payload.user_id, payload.book_id, payload.action)

        # Step 3: Generate fresh recommendations excluding all recent swipes
        user_idx = service.get_user_idx(payload.user_id)
        
        # Convert all recently swiped items (both likes and dislikes) to exclude them from recommendations
        # This prevents showing books the user has already interacted with
        exclude_catalog_indices = None
        all_swiped = session_state.get_all_recent_swiped(payload.user_id)
        if all_swiped:
            exclude_catalog_indices = service.book_ids_to_catalog_indices(list(all_swiped))
            logger.info(f"Excluding {len(exclude_catalog_indices)} recently swiped items from recommendations")
        ...
        recs, strategy = service.recommend(
            ...
            k=payload.k, 
            exclude_catalog_rows=exclude_catalog_indices
        )

        # Step 4: Filter out the book that was just swiped (shouldn't be in results, but be safe)
        # The recommendation engine already filters out rated items and session-swiped items
        ...
        # Step 5: Return response with replacement batch (clears old recommendations)
